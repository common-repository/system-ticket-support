...
$error = '';
if ( isset( $_POST['nonce'] ) && check_ajax_referer( 'sts_frontend_lock_template', 'nonce' )
) {
	...
	if ( user_can( $current_user, 'administrator' )
		...
		if ( $template->is_public == 1 ) {
			...
			if ( $result === false ) {
				$error = esc_html__( 'Can not set private for this template!!!', 'sts' );

			} else {
				ob_start();
				STS()->get_template( 'template/unlock-template.php', array(
					'template_id' => $id,
		...
		$error =
			esc_html__( "You have not permission to set private for this template!!", 'sts' );
	}
} else {
...
if ( $error == '' ) {
	sts_send_json(
		array(
			...
			'content_status' => esc_html__( 'Private', 'sts' )
		), 'update' );
} else {
	sts_send_json(
		array(
